...
def price_prediction(file_name):
	...
	# 예측 결과를 json 형식으로 변환
	historical_data_dict = {'date': df_month.index.strftime('%Y%m%d').tolist(), 'price': df_month['price'].tolist()}
	prediction_dict = {'date': forecast_dates.strftime('%Y%m%d').tolist(), 'price': prediction.tolist()}

	combined_data = {'historical_data': historical_data_dict, 'prediction_data': prediction_dict}

	# Convert the combined data dictionary to JSON
	combined_data_json = json.dumps(combined_data)
	
	# Save the JSON data to a file with the provided file_name
	with open(f'{file_name}.json', 'w') as json_file:
		json_file.write(combined_data_json)
